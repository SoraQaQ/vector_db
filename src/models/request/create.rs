    ...
    match request.index_type {
        Some(IndexType::HNSW) => {
            ...
            if request.max_elements.is_none() {
                return Err(ValidationError::new("max_elements is required for HNSW index type"));
            }
        }
        Some(_) => {
            // For non-HNSW types, max_elements must be None
            if request.max_elements.is_some() {
                return Err(ValidationError::new("max_elements is only allowed for HNSW index type"));
            }
        }
        None => {
